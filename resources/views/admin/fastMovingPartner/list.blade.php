...
<div id="content" class="container-fluid">
    <div class="card">
        <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
            <h5 class="m-0 ">Danh sách ảnh đối tác 2vexpress</h5>
        ...
        <div class="card-body">
            ...
            <div class="form-action form-inline py-3">
                <form action="{{ url('admin/fastMovingPartner/action') }}">
            ...
            <table class="table table-striped table-checkall">
                ...
                <tbody>
                    ...
                    <tr class="">
                        ...
                        <td>{{$temp}}</td>
                        <td class="wp-image-2vexpressImage"><img src="{{ asset($slider->image->image_url) }}" alt="" id="image-2vexpressImage"></td>
                        ...
                        <td>
                            <a href="{{ route('fastMovingPartner.edit',$slider->id) }}" class="btn btn-success btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="fa fa-edit"></i></a>
                           @if ($status == "trash")
                           <a href="{{ route('fastMovingPartner.delete',$slider->id) }}?status={{$status}}" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete" onclick="if(!confirm('Bạn có muốn xóa vĩnh viễn không?')) {event.preventDefault();}"><i class="fa fa-trash"></i></a>
                           @else
                           <a href="{{ route('fastMovingPartner.delete',$slider->id) }}" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete" onclick="if(!confirm('Bạn có muốn xóa tạm thời không?')) {event.preventDefault();}"><i class="fa fa-trash"></i></a>
                           @endif
